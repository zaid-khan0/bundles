    ...
    stage('Import') {
        script {
            if (status == "SUCCESS") {
                ...
                sh """#!/bin/bash
                ${DBCLIPATH}/databricks workspace export-dir ${DEV_DIR} /var/lib/jenkins/workspace/db_dir --profile myprofile
                   """
                
                sh """#!/bin/bash
                ${DBCLIPATH}/databricks workspace import-dir /var/lib/jenkins/workspace/db_dir /Workspace/Users/awsdatabricks00@gmail.com/gggg --profile myprofile
                   """
